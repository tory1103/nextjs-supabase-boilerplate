...
export default function Breadcrumbs(props: BreadcrumbsProps) {
	...
	const pathnameEndpoints = pathname.split('/').filter(Boolean);
			...
			<BreadcrumbList>
				{home && (
					<Fragment key='home'>
						...
						<BreadcrumbSeparator key='separator@home'>{separator}</BreadcrumbSeparator>
					</Fragment>
				)}

				{pathnameEndpoints.map((endpoint, index) => (
					<Fragment key={`${endpoint}@${index}`}>
						...
						{!(index === pathnameEndpoints.length - 1) && (
							<BreadcrumbSeparator key={`separator@${endpoint}@${index}`}>{separator}</BreadcrumbSeparator>
						)}
					</Fragment>
				))}
			</BreadcrumbList>
