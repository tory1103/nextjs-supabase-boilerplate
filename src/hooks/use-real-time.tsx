...
export default function RealTimeProvider(props: RealTimeProviderProps) {
	const defaultPresencePayload = useMemo(() => ({}), []);
	const defaultEvent = useMemo(() => 'message', []);

	const { children, channel, event = defaultEvent, presenceUID, presencePayload = defaultPresencePayload } = props;

	const supabase = createSupabaseCLClient();
	const [supabaseChannel, setSupabaseChannel] = useState<RealtimeChannel | undefined>(undefined);
	const [messages, setMessages] = useState<string[]>([]);
	const [presences, setPresences] = useState<RealTimeProviderProps['presencePayload'][]>([]);
			...
			realtimeChannel.on('presence', { event: 'leave' }, ({ leftPresences }) =>
				setPresences(presences =>
					presences.filter(presence =>
						leftPresences.some(leftPresence => leftPresence?.presence_ref !== presence?.presence_ref),
					),
				),
			);
	...
	const disconnect = useCallback(async (realtimeChannel: RealtimeChannel | undefined) => {
		if (!realtimeChannel) {
			return;
		}
	...
	useEffect(() => {
		...
		return () => {
			disconnect(realtimeChannel as any).catch(console.error);
		};
